...
function getSourceUrl(video) {
  ...
  switch (video.platform) {
    case 'tiktok': return video.source_url || `https://www.tiktok.com/@${video.channel || 'video'}/video/${id.replace(/^tiktok_/, '')}`;
    case 'instagram': return video.source_url || `https://www.instagram.com/p/${id.replace(/^ig_/, '')}/`;
    case 'reddit': return video.source_url || `https://www.reddit.com/search/?q=${encodeURIComponent(video.title || '')}`;
    default: return `https://www.youtube.com/watch?v=${id}`;
  }
}

function getSourceLabel(platform) {
  const labels = { youtube: 'Open on YouTube', tiktok: 'Open on TikTok', instagram: 'Open on Instagram', reddit: 'Search on Reddit' };
...
function isPending(value) { return !value || value === 'Analysis pending'; }
...
function InsightItem({ icon, label, value, tone }) {
...
function PendingInsight() {
  return <div className="vd-pending"><span aria-hidden="true">✦</span><strong>Luna is analyzing this content...</strong><small>Fresh intelligence will appear when the pipeline finishes.</small></div>;
}
...
export default function VideoDetail() {
  ...
  if (loading) return <div className="vd-loading"><span />Loading content intelligence...</div>;
  if (error) return <div className="vd-error">Unable to load this content. <span>{error}</span></div>;
  ...
  return (
    <div className="vd-page">
      <Link to="/videos" className="vd-back">← <span>Back to Top Content</span></Link>
      <header className="vd-header">
        <div className="vd-identity">
          <div className="vd-kicker">CONTENT INTELLIGENCE <span>✦</span></div>
          <h1>{video.title || 'Untitled content'}</h1>
          <div className="vd-meta-line"><span className="vd-platform-pill" style={{ '--platform-color': platformColor }}><PlatformIcon platform={video.platform} size={15} /> {getPlatformLabel(video.platform)}</span>{video.niche && <span className="vd-soft-pill">{video.niche}</span>}{video.channel && <span>by <strong>{video.channel}</strong></span>}{video.published_at && <span>• {new Date(video.published_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</span>}</div>
        </div>
      ...
      <section className="vd-section vd-transcript"><div className="vd-card-heading"><div className="vd-inline-title">🎙 Transcript <span className="vd-available">{video.transcript ? 'Available' : 'Not available'}</span></div>{video.transcript && <div className="vd-transcript-actions"><span>{wc.toLocaleString()} words</span><button type="button" onClick={() => exportTranscriptPDF(video)}>Download</button><button type="button" onClick={handleCopy}>{copied ? '✓ Copied' : 'Copy'}</button><button type="button" onClick={() => setTranscriptExpanded(!transcriptExpanded)}>{transcriptExpanded ? 'Collapse' : 'Expand'}</button></div>}</div>{video.transcript ? <><div className="vd-transcript-search"><input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search transcript..." />{searchTerm && <span>{matchCount} match{matchCount === 1 ? '' : 'es'}</span>}</div><div className={`vd-transcript-body ${transcriptExpanded ? 'is-expanded' : ''}`}>{transcriptParts.map((part, index) => <p key={index}>{part}</p>)}</div>{!transcriptExpanded && <button className="vd-show-transcript" type="button" onClick={() => setTranscriptExpanded(true)}>Show full transcript ↓</button>}</> : <div className="vd-pending vd-transcript-pending">Transcript is not available for this content.</div>}</section>
      <section className="vd-opportunity"><div><span className="vd-opportunity-icon">💡</span><div><small>LUNA CREATOR TOOLKIT</small><h2>Turn this trend into your next post</h2><p>Use this content&apos;s signals as a starting point for your next creative.</p></div></div><button className="vd-action vd-action-primary" type="button" onClick={() => setGenerateVideo(video)}>✦ Generate similar</button></section>
      <section className="vd-section vd-details"><div className="vd-inline-title">Content details</div><div className="vd-details-grid"><div><small>Platform</small><strong><PlatformIcon platform={video.platform} size={14} /> {getPlatformLabel(video.platform)}</strong></div><div><small>Published</small><strong>{video.published_at ? new Date(video.published_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : '—'}</strong></div><div><small>Content ID</small><strong>{video.video_id}</strong></div><div><small>Transcript</small><strong>{video.transcript ? `✓ ${wc.toLocaleString()} words` : 'Not available'}</strong></div><div><small>Last updated</small><strong>{video.updated_at ? new Date(video.updated_at).toLocaleDateString('en-GB') : '—'}</strong></div><div><small>Source</small><a href={getSourceUrl(video)} target="_blank" rel="noopener noreferrer">{getSourceLabel(video.platform)} ↗</a></div></div></section>
