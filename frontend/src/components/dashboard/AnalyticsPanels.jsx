...
export function TrendDistributionPanel({
  data
}) {
  const {
    t
  } = useTranslation();
  return <section className="dash-panel dash-chart-panel">
      <div className="dash-panel-heading"><h2><span aria-hidden="true">▥</span>{t('trendDistribution')}</h2></div>
      {data.length > 0 ? <div className="dash-bar-chart">
          <ResponsiveContainer width="100%" height={170}>
            <BarChart data={data.slice(0, 5)} layout="vertical" margin={{
          top: 1,
          right: 24,
          bottom: 0,
          left: 2
              ...
              <YAxis type="category" dataKey="name" width={106} tick={{
            fill: '#91a0be',
            fontSize: 10
        ...
        </div> : <div className="dash-chart-empty">{t('noTrendDistributionYet')}</div>}
    </section>;
}
export function NicheDistributionPanel({
  data
}) {
  const {
    t
  ...
  return <section className="dash-panel dash-chart-panel dash-niche-panel">
      <div className="dash-panel-heading"><h2><span aria-hidden="true">◉</span>{t('nicheDistribution')}</h2></div>
      {data.length > 0 ? <div className="dash-niche-body">
          ...
          <div className="dash-niche-legend">
            {data.slice(0, 4).map((item, index) => <div key={item.fullName}>
                <span style={{
        ...
        </div> : <div className="dash-chart-empty">{t('noNicheDistributionYet')}</div>}
    </section>;
}
export function PlatformDistributionPanel({
  data
}) {
  const {
    t
  ...
  const chartData = entries.map(([platform, videos]) => ({
    platform,
    videos
  }));
  return <section className="dash-panel dash-chart-panel dash-platform-panel">
      <div className="dash-panel-heading"><h2><span aria-hidden="true">◌</span>{t('platformDistribution')}</h2></div>
      {entries.length > 0 ? <div className="dash-platform-body">
          ...
          <div className="dash-platform-legend">
            {entries.map(([platform, videos]) => <div key={platform}>
                <PlatformIcon platform={platform} size={16} />
                <span>{platform}</span>
                <strong>{Math.round(videos / total * 100)}%</strong>
              </div>)}
          </div>
        </div> : <div className="dash-chart-empty">{t('noPlatformDistributionYet')}</div>}
    </section>;
}
export function ViralHeatmapPanel() {
  ...
  return <section className="dash-panel dash-heatmap-panel">
      <div className="dash-panel-heading"><h2><span aria-hidden="true">✧</span>{t('viralHeatmap')}</h2><span className="dash-panel-note">{t('comingSoon')}</span></div>
      <div className="dash-heatmap-empty">
        <div className="dash-heatmap-orbit" aria-hidden="true"><span /><span /><span /></div>
        <strong>{t('geographicSignalsAreNotIndexed')}</strong>
        <p>{t('lunaWillLightUpRegional')}</p>
      </div>
    </section>;
}